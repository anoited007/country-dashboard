...
from wazimap.data.utils import get_stat_data, calculate_median
# from wazimap.data.utils import get_stat_data, get_objects_by_geo, calculate_median
...
def get_ipv6_profile(geo, session):
    ...
    try:
        if geo.geo_level == 'country':
            ...
            view_ft_v6users, _ = get_stat_data(['type'], geo, session, table_name='ft_v6users_country_continent',
                                               recode=dict(REGION_RECODES), key_order=REGION_RECODES.values())
    ...
    try:
        ...
        view_ft_v4_v6, _ = get_stat_data(['type'], geo, session, table_name='ft_v4_v6', recode=dict(REGION_IP),
                                         key_order=REGION_IP.values())
    except Exception as e:
